...
demo:
	SEED_DIR=$(SEED_DIR) python -m cedx_fleet.cli demo
...
probe-budget:
	SEED_DIR=$(SEED_DIR) python -m cedx_fleet.cli probe-budget

# Exit 0 ONLY if mutating/deleting a past audit entry is refused.
probe-append-only:
	python -m cedx_fleet.cli probe-append-only

# Exit 0 ONLY if running demo twice produces no duplicate outputs/exceptions/approvals.
probe-idempotency:
	SEED_DIR=$(SEED_DIR) python -m cedx_fleet.cli probe-idempotency
